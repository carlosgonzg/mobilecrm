...
var sendInvoice = function (invoice, mails, cc, file, fileName) {
	...
	bringTemplateData('/email/templateInvoice.html')
		.then(function (body) {
			...
			body = body.replace('<pono>', invoice.pono && invoice.pono != ''? 'With PO Number: ' + invoice.pono : 'Without PO Number. Please provide PO Number for this Invoice');
...
var sendWorkOrder = function (workOrder, mails, dirname, file, fileName, buttonSend) {
	var deferred = q.defer();
...
var sendWorkOrderUpdate = function (workOrder, mails, user, company, file, fileName) {
	var deferred = q.defer();
