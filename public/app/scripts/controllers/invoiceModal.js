...
.controller('InvoiceModalCtrl', function ($scope, $rootScope, $location, $timeout, toaster, $uibModalInstance, User, Item, ServiceOrder, WorkOrder, dialogs, Invoice, Company, data) {
	console.log(data)
	$scope.invoice = data.invoice;
	$scope.listStatus = data.status;
	$scope.listCompany = data.companies.data;
	var response = {};
	...
	$scope.saveBranch = function () {
		var branchId = $scope.invoice.client.branch._id;
		delete $scope.invoice.client.account.password;
		$scope.invoice.save()
		.then(function (data) {
			new User().filter({ 'branch._id': branchId })
...
.controller('InvoiceModalCtrl', function ($scope, $rootScope, $location, $timeout, toaster, $uibModalInstance, User, Item, ServiceOrder, WorkOrder, dialogs, Invoice, Company, data) {
	...
	$scope.send = function(){
	 ...
	 $scope.cancel = function () {
	  ...
	  $scope.saveAndClose = function (result) {
	    $uibModalInstance.close($scope.invoice || response);
	  };

	if($scope.invoice.client){
		$scope.clientChanged($scope.invoice.client);
	}
});
